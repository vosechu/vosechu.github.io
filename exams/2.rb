...
section.questions << Question.new(%{If you have a file called `index` in your public folder, can you override that file with `get "index" do; end`?}, %{No.}, [%w{}, %w{}, %w{}])

# Use a reloader to make development faster
section.questions << Question.new(%{How do you enable reloading if you've already added sinatra-contrib to your Gemfile?}, %{require "sinatra/reloader" if development?}, [%w{}, %w{}, %w{}])
...
section.questions << Question.new(%{How do you see a list of all rake tasks}, %{rake -T}, [%w{}, %w{}, %w{}])
...
section.questions << Question.new(%{How do you see a list of generators that are available to you?}, %{rails generate -h}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{What is a scaffold?}, %{A scaffold is a generator that creates a model, migration, and controller sufficient for use as a RESTful API}, [%w{}, %w{}, %w{}])
...
section.questions << Question.new(%{In the controller, how do you redirect to the winningest entries path?}, %{redirect_to winning_entries_path}, [%w{}, %w{}, %w{}])

# Add routes to the Router
section.questions << Question.new(%{How do you create a route that allows the 7 standard controller actions for an object?}, %{resources 'entries'}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{How do you create a route that matches '/submit' with the method "POST"}, %{post "submit", :to => 'entries#submit'}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{How do you see a list of all the routes?}, %{rake routes}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{How do you create a route that matches '/'}, %{root :to => 'entries#index'}, [%w{}, %w{}, %w{}])

# Create SCSS and Coffeescript assets and include them in the layout
section.questions << Question.new(%{Where do coffeescript files live in the Rails folder structure?}, %{RAILS_ROOT/app/assets/javascripts}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{Where do scss files live in the Rails folder structure?}, %{RAILS_ROOT/app/assets/stylesheets}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{How do you include a javascript file before all the other javascript files}, %{Add that file to the manifest above 'require_tree .'}, [%w{}, %w{}, %w{}])

# Use simple relationships to link models together
section.questions << Question.new(%{If model A has 'belongs_to :b' in the model file, which one has an extra _id column in the database table?}, %{Model A}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{If model A has 'has_many :cs' in the model file, which one has an extra _id column in the database table?}, %{Model C}, [%w{}, %w{}, %w{}])

# Deploy projects to Heroku
section.questions << Question.new(%{If you need to get data from your database to Heroku, how do you do it?}, %{heroku db:push}, [%w{}, %w{}, %w{}])
...
section.questions << Question.new(%{How do you connect to MySQL via the network from the command line?}, %{mysql -u root -p -h localhost}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{How do you connect to Sqlite3 via the command line?}, %{sqlite3 db/development.sqlite3}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{What does it mean to connect via a socket?}, %{MySQL supports connecting via a Unix socket file instead of connecting over the network (great for conferences).}, [%w{}, %w{}, %w{}])

# Execute SELECT, INSERT, UPDATE, and DELETE statements from the command-line
section.questions << Question.new(%{What command is used to find and return data?}, %{SELECT}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{What sub-expression do you use to filter data?}, %{WHERE}, [%w{}, %w{}, %w{}])

# Understand relationships and be able to discuss on a whiteboard or pseudo-code
section.questions << Question.new(%{When a model `has_many` other models, how does that affect the database?}, %{In some databases (InnoDB/Postgres) it'll add a foreign key, otherwise it doesn't affect it}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{When a model Car `belongs_to` a Driver, how does that affect the database?}, %{The cars table must include a column driver_id}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{What sub-expression do we use to add results from another table (not results from another query)}, %{LEFT JOIN or INNER JOIN}, [%w{}, %w{}, %w{}])
section.questions << CodeQuestion.new(%{If you need to find all cars that belong to a driver (id: 3), how do you do that?}, %{SELECT *\nFROM cars\nWHERE driver_id = 3}, [%w{}, %w{}, %w{}], 'sql')
section.questions << CodeQuestion.new(%{If you need to find all cars that have a driver with hair_type = mullet, how do you do that?}, %{SELECT cars.*\nFROM cars\nLEFT JOIN drivers\n  ON drivers.id = cars.driver_id\nWHERE drivers.hair_type = 'mullet'}, [%w{}, %w{}, %w{}], 'sql')
section.questions << CodeQuestion.new(%{If you need to find all drivers that have a particular car (id: 42), how do you do that?}, %{SELECT cars.*\nFROM cars\nLEFT JOIN drivers\n  ON drivers.id = cars.driver_id\nWHERE cars.id = 42}, [%w{}, %w{}, %w{}], 'sql')

# Create migrations in Rails or Sinatra
section.questions << Question.new(%{How do you create a migration from the Rails command line?}, %{rails generate migration <name>}, [%w{}, %w{}, %w{}])

# Understand ORDER and LIMIT
section.questions << Question.new(%{How do you restrict the number of results coming from the database?}, %{LIMIT n}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{How do you order data?}, %{ORDER BY}, [%w{}, %w{}, %w{}])
...
section.questions << Question.new(%{What files are required to be in your project to make a backbone.js website (use a CDN for vendored assets)?}, %{Only index.html and an app.js}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{Do you need jQuery to use Backbone?}, %{For DOM manipulation it requires jQuery or Zepto and json2.js}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{What order should you include your javascript files?}, %{jquery, jquery-migrate, underscore, backbone}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{Should your backbone includes be in the head or the footer of the page?}, %{in the footer for performance reasons}, [%w{}, %w{}, %w{}])

# Create a backbone app from scratch in multiple Ruby frameworks
section.questions << Question.new(%{How do you create a backbone app in Sinatra?}, %{put an index.erb in /views, render it from app.rb, and put the backbone includes in the layout}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{How do you create a backbone app in Rails?}, %{create a static controller and put the backbone includes in the layout}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{What does the rails-backbone gem give us?}, %{scaffolding support and files for the asset pipeline}, [%w{}, %w{}, %w{}])

# Create models, views, and collections in one file
section.questions << Question.new(%{What is the main purpose of a model in Backbone?}, %{Persistence, validation, and data storage, triggering events on change}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{What is the main purpose of a collection?}, %{Fetching data and triggering events when something is added or removed}, [%w{}, %w{}, %w{}])
section.questions << Question.new(%{What is the main purpose of a view in Backbone?}, %{To orchestrate between the DOM and any attached collections or models}, [%w{}, %w{}, %w{}])
